if ! [ -x "$(command -v x86_64-w64-mingw32-objdump)" ]
then
    >&2 echo "x86_64-w64-mingw32-objdump could not be found. Please ensure binutils-mingw-w64-x86-64 is installed."
    exit -1
fi
...
version="$(x86_64-w64-mingw32-objdump -v | sed -n 's/^.*[^0-9]\([0-9]*\.[0-9]*\.[0-9]*\).*$/\1/p')"
...
for dll_path in "${results[@]}"; do
    ...
    ( # Run bunch of commands, output to .def file
        ...
        # Get objdump data
        objdump_output=$(x86_64-w64-mingw32-objdump -p "$dll_path")
    ...
    timeout 10s x86_64-w64-mingw32-windres -i "$dll_path" -O coff -o "$output_folder/$dll_path.res" 2> /dev/null
    if [ $? -eq 0 ]; then
        # Compile our output DLL, using (static) .C template and (generated) .def file
        x86_64-w64-mingw32-gcc -shared -mwindows -o "$output_folder/$dll_path" "$output_folder/$dll_path.def" "$output_folder/$dll_path.res" ../template.c
        # Remove redundant .def/.rsrc files
        rm "$output_folder/$dll_path.def" "$output_folder/$dll_path.res";
    else
        # Compile our output DLL, using (static) .C template and (generated) .def file
        x86_64-w64-mingw32-gcc -shared -mwindows -o "$output_folder/$dll_path" "$output_folder/$dll_path.def" ../template.c
        # Remove redundant .def/.rsrc files
